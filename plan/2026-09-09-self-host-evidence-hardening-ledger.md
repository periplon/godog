...
- 2026-09-09: The stricter checker exposed an overly narrow assumption: legitimate reviews used nonempty structured `checked_scope` and `test_evidence` fields because the workflow did not prescribe legacy string field names. A focused regression failed with `review compiler.json scope is missing`.
- 2026-09-09: Added validated support for both structured and legacy evidence forms. The completed seven-task run at `godog-runs/selfhost-01` then passed the stricter checker with `self-host evidence verified`.
